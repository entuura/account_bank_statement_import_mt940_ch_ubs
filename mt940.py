...
import re
from flectra.addons.account_bank_statement_import_mt940_base.mt940 import MT940
...
class MT940Parser(MT940):
    ...
    #     r'(?P<rzbid>\w{16}){0,1}'
    # )
    tag_61_regex = re.compile(r'^(?P<date>\d{10})'
                              r'(?P<sign>[R]{0,1}[CD])'
                              r'(?P<amount>\d+,\d+)'
                              r'(?P<raw>\w+//)'
                              r'(?P<reference>\w{0,16})'
    ...
    tag_61_regex_2 = re.compile(r'^(?P<date>\d{10})'
                              r'(?P<sign>[R]{0,1}[CD])'
                              r'(?P<amount>\d+,)'
                              r'(?P<raw>\w+//)'
                              r'(?P<reference>\w{0,16})'
    ...
    def handle_tag_61(self, data):
        ...
        if not re_61:
            re_61 = self.tag_61_regex_2.match(data)
            if not re_61:
        ...
        self.current_transaction['note'] = parsed_data['description']
    ...
    def handle_tag_86(self, data):
        ...
        """
        if len(data) > 4:
            self.current_transaction["name"] = data[4:]
        else:
